...
unzip FLAME2020.zip -d render/data/FLAME2020/
rm -rf FLAME2020.zip
mv render/data/FLAME2020/Readme.pdf render/data/FLAME2020/Readme_FLAME.pdf


wget 'https://files.is.tue.mpg.de/tbolkart/FLAME/FLAME_masks.zip' -O './FLAME_masks.zip' --no-check-certificate --continue
unzip FLAME_masks.zip -d render/data/FLAME2020/
rm -rf FLAME_masks.zip

echo -e "\n${COLOR}Downloading Mesh..."
wget -O mesh.zip "https://keeper.mpdl.mpg.de/f/f158a430ef754edba5ec/?dl=1"
unzip mesh.zip -d render/data/
mv render/data/mesh/* render/data/
rm -rf render/data/mesh
rm -rf mesh.zip
